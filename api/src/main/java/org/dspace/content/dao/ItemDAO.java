...
public interface ItemDAO extends DSpaceObjectLegacySupportDAO<Item>
{
    ...
    /**
     * Get all Items that have a metadatafield set with a value similar to the value provided. This method uses the
     * Levenshtein algorithm to do a fuzzy search. The Levenshtein algorithm calculates the number of singel characters
     * that needs to be changed, deleted or swapped to change one value into another one. The attribute
     * {@code maxDisance} allows you to configure the maximum number of allowed changes to still recognize to values as
     * being similar. Please note that for performance reasons only the first 255 characters of each value are compared.
     * @param context context
     * @param field The metadatafield that is going to be compared.
