...
public class ItemDAOImpl extends AbstractHibernateDSODAO<Item> implements ItemDAO
{
    ...
    public Iterator<Item> detectDuplicateItems(Context context, MetadataField field, String value, int maxDistance)
            throws SQLException
    {
        // Postgresql's implementation of Levenshtein supports 255 characters only for performance reasons.
        Query query = createQuery(context,
                                  "SELECT item FROM Item item " +
                                          "JOIN item.metadata field " +
                                          "WITH field.metadataField.id = :field_id " +
                                          "WHERE levenshtein_less_equal(lower(substring(:value, 0, 254)), lower(substring(field.value, 0, 254)), :distance) <= :distance " +
                                          "ORDER BY levenshtein_less_equal(lower(substring(:value, 0, 254)), lower(substring(field.value, 0, 254)), :distance), text_value ASC nulls last"
//                                          "WHERE levenshtein(:value, field.value) <= :distance"
        );
        query.setParameter("field_id", field.getID());
        query.setParameter("value", value);
        query.setParameter("distance", maxDistance);
        return iterate(query);
    }
}
